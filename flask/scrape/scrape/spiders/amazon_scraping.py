...
class ReviewspiderSpider(scrapy.Spider):
    ...
    myBaseUrl = ''
    start_urls = []
    #'FEED_URI': 'tutorial/outputfile.json',
    custom_settings = {'CLOSESPIDER_TIMEOUT' : 15} # How long the spider should run.
    ...
    def parse_page(self, response):
        ...
        for (name, title, Review, Verified, date, rating, helpful_count) in zip(names, reviewTitles, reviewBody, verifiedPurchase, postDate, starRating, helpful):
            
            # Getting the Next Page URL for futher scraping.
            #next_urls = response.css('.a-last > a::attr(href)').extract_first()
            
            yield MyItem(names=name, reviewTitles=title, reviewBody=Review, verifiedPurchase=Verified, postDate=date, starRating=rating, helpful=helpful_count)
